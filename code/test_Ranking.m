...
for k =1:length(kname) % kernel for loop, select different kernel
    ...
    savename = ['Result_' dataset_name '_' AlgoOption.name '_' Featurename '_' partition_name '_' kname{k} '_' timstr '.mat'];
    display(savename);
    if ~iscell(Feature)
        tmpF{1} = Feature;
        Feature = tmpF;
...
%                 nothing
    ...
    for idx_partition=1:length(Partition) % partition loop        
        ...
        for iset = 1:2
            ...
            if AlgoOption.isCOV
                train = feature_set(idx_train,idx_train,:); % training set
                test = feature_set(idx_test,idx_test,:); % testing set
            else
...
%                 case {'PRDC'}
%                     [r, dis] = compute_rank_PRDC(algo,test,ix_test_gallery,gID(idx_test));
                otherwise
                    [r, dis] = compute_rank2(algo, train, test, ix_test_gallery, gID(idx_test));
            end
